...
def upload(client , pressed, shift, conn):
    try:
        ...
        sample = ServerSecurity.decrypt(data,shift)
        analyst = AiAnalyzer.analyze_with_gemma(sample, file)
        if analyst['state'] == "PASS":
            conn.upload_file(pressed, data, file)
            client.sendall(ServerSecurity.encrypt("Uploaded Successfully and Verified by AI".encode(),shift))
        else:
            reason = analyst.get("main_reason", "Security Block")
            client.sendall(ServerSecurity.encrypt(f"BLOCKED by Server AI. Reason: {reason}".encode(),shift))
    except:
        print("[Ollama] Analyse error")

def delete(client , file_name, shift, conn):
    msg = conn.del_file(file_name)
    msg = ServerSecurity.encrypt(msg.encode(),shift)
    client.sendall(msg)
...
def download(client, file_name, shift, conn):
    ...
    if content:
        ...
        client.sendall(ServerSecurity.encrypt(size,shift))
        client.sendall(ServerSecurity.encrypt(content[1],shift))
        print(f"{file_name} Successfully sent to donwload in client")
    else:
        client.sendall(ServerSecurity.encrypt("not existing".encode(), shift))
...
def backup_system(client,username,shift):
    ...
    while True:
        try:
            ...
            if not pressed:
                print(f"[BACKUP] Client {username} disconnected.")
                break
            pressed = ServerSecurity.decrypt(pressed, shift)
            ...
            match action:
                case "get_files":
                    ...
                    file_list = ServerSecurity.encrypt(DBHandle.capsulize(file_list),shift)
...
def login(username, password, addr):
    ...
    hashed_password = ServerSecurity.hash_sha(password.encode())
...
def two_step(client, username, password, shift):
    ...
    code = ServerSecurity.send_verification_code(email)
    email_sent = time.time()
    while True:
        code_entry = ServerSecurity.decrypt(client.recv(1024), shift).decode()
        if not code_entry:
            print("Client Disconnected")
            break
        current_time = time.time()
        if email_sent + EXPIRE_TIME < current_time:
            client.sendall(ServerSecurity.encrypt("expired".encode(), shift))

        if code_entry == code:
            break
        client.sendall(ServerSecurity.encrypt("fail".encode(), shift))
    client.sendall(ServerSecurity.encrypt("success".encode(), shift))
    ...
    try:
        while True:
            ...
            if not info:
                ...
                break
            info = ServerSecurity.decrypt(info,shift)
            ...
            match action:
                case "login":
                    ...
                    print(result)
                    client.sendall(ServerSecurity.encrypt(result.encode(), shift))
                    if result == "success":
...
def handle_sessions():
    while True:
        ...
        Encrypt_Manager = ServerSecurity.RSAKeyManager()
